...
pub fn loading_animation_script() -> PreEscaped<String> {
    PreEscaped(r#"
        ...
        document.body.addEventListener('htmx:xhr:loadstart', function(evt) {
            ...
            let interval = setInterval(() => {
                if (currentStep < steps.length - 1) {
                    ...
                    if (statusText) {
                        switch(currentStep) {
                            case 1: statusText.textContent = 'Checking branch propagation...'; break;
                            case 2: statusText.textContent = 'Verifying channel status...'; break;
                            case 3: statusText.textContent = 'Finalizing...'; break;
                        }
                    }
                }
            }, 400);
            ...
            document.body.dataset.loadingInterval = interval;
        });

        document.body.addEventListener('htmx:xhr:loadend', function(evt) {
            // Clear the interval
            if (document.body.dataset.loadingInterval) {
                clearInterval(document.body.dataset.loadingInterval);
            }
        });
    "#.to_string())
}
        ...
        html lang="en" data-theme=(config.theme) {
            ...
            body {
                div.container {
                    ...
                    div.loading-container.htmx-indicator id="loading" {
                        div.loading-content {
                            div.spinner {}
                            p.loading-status { "Fetching PR details..." }
                            div.loading-steps {
                                div.step.active id="step-pr" { "📄 Loading PR information" }
                                div.step id="step-branches" { "🌿 Checking Git branches" }
                                div.step id="step-channels" { "📡 Verifying channels" }
                                div.step id="step-complete" { "✅ Complete" }
                            }
                        }
                    }
                ...
                script { (loading_animation_script()) }
            }
        }
    }
}
...
pub fn pr_page(config: &Config, pr_number: u64) -> Markup {
    html! {
        ...
        html lang="en" data-theme=(config.theme) {
            ...
            body hx-get=(format!("/pr?pr={}", pr_number)) hx-trigger="load" hx-target="#results" hx-indicator="#loading" {
                div.container {
                    header {
                        h1 { "🔍 " (config.owner) "/" (config.repo) " Pull Request Tracker" }
                    ...
                    div.loading-container id="loading" style="display: flex;" {
                        div.loading-content {
                            div.spinner {}
                            p.loading-status { "Fetching PR #" (pr_number) "..." }
                            div.loading-steps {
                                div.step.active { "📄 Loading PR information" }
                                div.step { "🌿 Checking Git branches" }
                                div.step { "📡 Verifying channels" }
                                div.step { "✅ Complete" }
                            }
                        }
                    }
